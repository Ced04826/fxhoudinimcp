...
class TestTheProbeAnswersMore:
    def test_template_types_and_factory_expressions_come_from_the_same_probe(self, monkeypatch):
        ...
        result = graph._parm_names_for_type(scratch, MagicMock())
        types = {name: shape["type"] for name, shape in result.shapes.items()}
        shipped = result.expressions
...
def _shape(template: str, components: int = 1) -> dict:
    return {"type": template, "components": components, "is_tuple": components > 1}


def _knowledge(parm_names, tuple_names, shapes, expressions=None):
    """What the fork's probe returns: the five-tuple plus shapes and expressions."""
    connectors = {"inputs": [], "outputs": []}
    return graph._TypeKnowledge(
        set(parm_names), set(tuple_names), {}, [], connectors, shapes, expressions or {}
    )


@pytest.fixture(autouse=True)
def _fresh_type_cache(monkeypatch):
    """Probe results are cached per session; each test probes its own types."""
...
def validating_build(monkeypatch):
    ...
    def knowledge(scratch, resolved):
        shapes = {
            "input": _shape("Int"),
            "name": _shape("String"),
            "t": _shape("Float", 3),
        ...
        return _knowledge({"input", "name", "tx", "ty", "tz"}, {"t"}, shapes)
...
def ray_type(monkeypatch):
    ...
    def knowledge(scratch, resolved):
        ...
        expressions = {"dir": {"dirx": "@N.x", "diry": "@N.y", "dirz": "@N.z"}}
        return _knowledge({"dirx", "diry", "dirz", "entity"}, {"dir"}, shapes, expressions)
...
class TestTheBuildSaysWhatDidNotTake:
    ...
    def test_an_expr_value_is_set_as_an_expression_in_its_language(self, building, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda scratch, resolved: _knowledge(
                {"input", "seed"}, set(), {"input": _shape("Int"), "seed": _shape("Int")}
            ),
        )
