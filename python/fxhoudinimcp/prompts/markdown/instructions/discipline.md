...
3.  VERIFY, THEN CLAIM. After any build or change, `verify_network(parent)` (the middle-click-everything pass); read `error_nodes` and the geometry counts. At visual milestones `capture_viewport` (framed on the part; `shading="smooth_wire"` shows the wiring; several views in one call) and LOOK at the image. A tool returning success is not evidence.
