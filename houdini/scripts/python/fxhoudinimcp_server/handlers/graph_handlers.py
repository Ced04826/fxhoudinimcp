...
_TYPE_KNOWLEDGE: dict[tuple, dict] = {}
...
def _type_knowledge(scratch: hou.Node, node_type) -> dict[str, dict]:
    """Instantiate a type once to learn its parameter names, shapes and types.

    Names alone caught a misspelling and nothing else, so a spec that named a
    real three-float parameter and handed it two numbers validated cleanly and
    then failed during the build -- after other nodes had been created. The
    component count and template type are knowable here, so they are checked
    here.
    """
    ...
    for index, spec in enumerate(nodes):
        ...
        if knowledge:
            for parm_name, parm_value in (spec.get("parms") or {}).items():
                info = knowledge.get(parm_name)
                if info is None:
                    close = get_close_matches(parm_name, sorted(knowledge), n=3, cutoff=0.5)
                    hint = f" Did you mean: {close}?" if close else ""
                    errors.append(
